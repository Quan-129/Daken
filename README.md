...
- **Identity UI "Zen Night" (Thẻ Căn Cước Đặc Vụ)**: 
  ...
  - **High-Contrast Agent ID & Security**: Mã số định danh (#ID) màu Trắng tinh khiết (`#ffffff`) phả hào quang Cyan, rực sáng và sắc nét. Đặc biệt, hệ thống tích hợp **Giao thức Bảo mật ID**: Chỉ Admin tối cao mới có quyền can thiệp và thiết lập ID số đẹp (`999999`), ngăn chặn mọi hành vi mạo danh thông qua Console.
  ...
  - **Quick Sync Relocation**: Nút "Đồng bộ cưỡng bức" (🔄) được tích hợp trực tiếp vào thẻ Profile, xóa bỏ sự rườm rà của giao diện cũ, cho phép đặc vụ làm mới dữ liệu Cloud chỉ với một lần nhấn.
...
- **Kiến trúc JLPT Đa Tầng & Hệ số Điểm Cân Bằng (v0.11.0)**:
  ...
  - **Hệ số Công bằng (Score Scaling Factor)**: Thuật toán điều tiết điểm số thông minh dành cho chế độ Học (Session). Đảm bảo Rank S luôn giữ giá trị thực bất kể bài học dài hay ngắn.
  - **Review Mode Exemption & Unlimited Score**: Đặc biệt, chế độ Tổng ôn được miễn trừ hoàn toàn khỏi hệ số điều tiết và gỡ bỏ mọi giới hạn điểm số cũ (3000đ). Giờ đây, bạn có thể tích lũy hàng chục nghìn điểm trong một phiên ôn tập dài hơi và lưu danh trực tiếp lên bảng vàng thành tích.
